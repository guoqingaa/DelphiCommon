...
type
  ...
  TXPrintDialog = class(TForm)
    ...
    Label10: TLabel;
    GroupBox6: TGroupBox;
    RadioMono: TRadioButton;
    RadioColor: TRadioButton;
...
procedure TXPrintDialog.FormCreate(Sender: TObject);
begin
...
procedure TXPrintDialog.DrawPage(mfPage: TMetafile);
begin
  Image.Picture.Metafile.Assign(mfPage);
  Refresh;
end;

procedure TXPrintDialog.SetConfig(Value : TXPrintConfig);
begin
  FConfig := Value;

  if Value.UseColor then
...
// big preview
procedure TXPrintDialog.PreviewClick(Sender: TObject);
begin
  if Assigned(FOnPreview) then
    FOnPreview(Self);
end;

procedure TXPrintDialog.RefreshPreview;
const
  MIN_MARGIN = 20;
var
  dAspectRatio,
...
procedure TXPrintDialog.PaintPreviewPaint(Sender: TObject);
begin
...
procedure TXPrintDialog.EditMarginExit(Sender: TObject);
begin
...
procedure TXPrintDialog.EditMarginChange(Sender: TObject);
...
procedure TXPrintDialog.CheckFitInPageClick(Sender: TObject);
begin
...
procedure TXPrintDialog.RadioOrientationClick(Sender: TObject);
begin
...
procedure TXPrintDialog.ComboPrinterClick(Sender: TObject);
begin
...
procedure TXPrintDialog.SpinCopiesChange(Sender: TObject);
begin
  Printer.Copies := SpinCopies.Value;
end;

procedure TXPrintDialog.RadioColorClick(Sender: TObject);
begin
  if RadioMono.Checked then
    FConfig.UseColor := False
  else
    FConfig.UseColor := True;
end;
