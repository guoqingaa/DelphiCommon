...
uses
  ...
  SMS_Columns, ComCtrls, ExtCtrls, Menus, //AppTypes,
...
begin
  ...
  FSelectedColor := clBlue;//SELECTED_COLOR;
  ...
  begin
    ...
    begin
      ...
      if Item.Index mod 2 = 1 then
        Brush.Color := clGray//EVEN_COLOR
      else
        Brush.Color := clWhite;//ODD_COLOR;
    end;
    //-- background
    FillRect(Rect);
    //-- icon
...
begin
  ...
  if Selected then
  begin
    FSelectedLeft:= TColumnItem(Item.Data);
  end
  else
    FSelectedLeft:= nil;
end;
...
begin
  ...
  begin
    //.. List내용 바꾸기
    aColumnItem := TColumnItem(Items[iFlag].Data);
    aColumnTypes.FieldName:= Trim( TColumnItem(Items[iFlag].Data).FieldName );
//    TColumnItem(FColumns.Defaults[i]).FieldName;
    ...
    aColumnTypes.UseGradationEffect := aColumnItem.UseGradationEffect;
    aColumnTypes.GradationMaxValue := aColumnItem.GradationMaxValue;
    aColumnTypes.GradationMinValue := aColumnItem.GradationMinValue;
    aColumnTypes.UseMaxColor := aColumnItem.UseMaxColor;
    aColumnTypes.UseMinColor := aColumnItem.UseMinColor;
    aColumnTypes.MaxValue := aColumnItem.MaxValue;
    aColumnTypes.MinValue := aColumnItem.MinValue;
    aColumnTypes.MaxColor := aColumnItem.MaxColor;
    aColumnTypes.MinColor := aColumnItem.MinColor;
    //..
    ...
    begin
      if TColumnItem(FColumns.ViewList[i]) = FListSelected.Selected.Data then
      begin
        aColumnItem := TColumnItem(FColumns.ViewList[i]);
        TColumnItem(FColumns.ViewList[i]).Title:= aColumnTypes.Title;
        TColumnItem(FColumns.ViewList[i]).Width:= StrToInt(aColumnTypes.Width);
        TColumnItem(FColumns.ViewList[i]).Scale:= StrToInt(aColumnTypes.Scale);

        aColumnItem.UseGradationEffect := aColumnTypes.UseGradationEffect;
        aColumnItem.GradationMaxValue := aColumnTypes.GradationMaxValue;
        aColumnItem.GradationMinValue := aColumnTypes.GradationMinValue;

        aColumnItem.UseMaxColor := aColumnTypes.UseMaxColor;
        aColumnItem.UseMinColor := aColumnTypes.UseMinColor;
        aColumnItem.MaxValue := aColumnTypes.MaxValue;
        aColumnItem.MinValue := aColumnTypes.MinValue;
        aColumnItem.MaxColor := aColumnTypes.MaxColor;
        aColumnItem.MinColor := aColumnTypes.MinColor;
...
begin
  ...
  case TComponent(Sender).Tag of
    ...
    begin
      ...
      aColumn := TColumnItem(FListFields.Selected.Data);
      ...
      begin
        ...
        begin
          ...
          begin
            Data:= FColumns[i];
            Caption:= (FColumns[i] as TColumnItem).FieldName;
            SubItems.Add((FColumns[i] as TColumnItem).Title);
    ...
    begin
      ...
      begin
        aColumn := TColumnItem(FListFields.Items[i].Data);
    ...
    300:  // unselect an item
      begin
        ...
        begin
          ...
          aColumn := TColumnItem(FListSelected.Selected.Data);
          if FColumns.FindFixedDeatails(aColumn.FieldName) then Exit;
    ...
    400:
      begin
        ...
        begin
          aColumn := TColumnItem(FListSelected.Items[i].Data);
          if FColumns.FindFixedDeatails(aColumn.FieldName) then Continue;
...
begin
  ...
  //고정 필드를 위한 작업
  aColumn := TColumnItem(FListSelected.Items[iMove].Data);
  if FColumns.FindFixedDeatails(aColumn.FieldName) then Exit;
...
begin
  ...
  for i:=0 to FColumns.ViewList.Count-1 do
  begin
    aColumn := TColumnItem(FColumns.ViewList[i]);
    if not(FColumns.FindNonVisibles(aColumn.FieldName)) then
    begin
      aItem:= FListSelected.Items.Add;
      aItem.Data:= TColumnItem(FColumns.ViewList[i]);
      aItem.Caption:= TColumnItem(FColumns.ViewList[i]).FieldName;
      aItem.SubItems.Add(TColumnItem(FColumns.ViewList[i]).Title);
      aItem.SubItems.Add(IntToStr(TColumnItem(FColumns.ViewList[i]).Width) );
      aItem.SubItems.Add(IntToStr(TColumnItem(FColumns.ViewList[i]).Scale));
      aItem.SubItems.Add('');
    end;
  end;
  ...
  begin
    ...
    for j:=0 to FColumns.ViewList.Count-1 do
      if FColumns[i] = TColumnItem(FColumns.ViewList[j]) then
      begin
        bFlag:= False;
        Break;
      end;
    ...
    if bFlag then
    begin
      aColumn := TColumnItem(FColumns.Items[i]);
      if not(FColumns.FindNonVisibles(aColumn.FieldName)) then
      begin
        aItem:= FListFields.Items.Add;
